...
function test_clone_encryption() {
  ...
  # write 1MB plaintext
  sudo dd if=/tmp/testdata1 of=$RAW_DEV oflag=direct bs=1M count=1

  # clone (luks1)
  rbd snap create testimg@snap
  rbd snap protect testimg@snap
  rbd clone testimg@snap testimg1 --encryption-format luks1 --encryption-passphrase-file /tmp/passphrase
  ...
  rbd clone testimg1@snap testimg2 --encryption-format luks2 --encryption-passphrase-file /tmp/passphrase
  ...
  # flatten
  rbd flatten testimg2 --encryption-passphrase-file /tmp/passphrase
