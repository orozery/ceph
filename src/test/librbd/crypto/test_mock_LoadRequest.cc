...
struct TestMockCryptoLoadRequest : public TestMockFixture {
  ...
  void expect_clone_format() {
    mock_parent_encryption_format = new MockTestEncryptionFormat("clone");
    EXPECT_CALL(*mock_encryption_format, clone()).WillOnce(
            Return(mock_parent_encryption_format));
  }
