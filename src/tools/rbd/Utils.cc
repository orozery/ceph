...
int get_encryption_options(const boost::program_options::variables_map &vm,
  ...
  if (vm.count(at::ENCRYPTION_PREFIX + at::ENCRYPTION_PASSPHRASE_FILE)) {
    has_passphrase = true;
    std::string passphrase_file =
            vm[at::ENCRYPTION_PREFIX +
               at::ENCRYPTION_PASSPHRASE_FILE].as<std::string>();
  ...
  if (format_options) {
    ...
    if (vm.count(at::ENCRYPTION_PREFIX + at::ENCRYPTION_CIPHER_ALG)) {
      alg = vm[at::ENCRYPTION_PREFIX + at::ENCRYPTION_PASSPHRASE_FILE].as<
              librbd::encryption_algorithm_t>();
    }

    if (vm.count(at::ENCRYPTION_PREFIX + at::ENCRYPTION_FORMAT)) {
      std::string format_str =
              vm[at::ENCRYPTION_PREFIX + at::ENCRYPTION_FORMAT].as<std::string>();
      if (format_str == "luks1") {
        if (!has_passphrase) {
          std::cerr << "rbd: missing "
                    << at::ENCRYPTION_PREFIX + at::ENCRYPTION_PASSPHRASE_FILE
                    << std::endl;
          return -EINVAL;
        }
    ...
    spec.format = RBD_ENCRYPTION_FORMAT_LUKS;
    spec.opts_size = sizeof(librbd::encryption_luks_format_options_t);
    spec.opts = malloc(spec.opts_size);
    memset(spec.opts, 0, spec.opts_size);
    auto luks_opts =
            new(spec.opts) librbd::encryption_luks_format_options_t;
    luks_opts->passphrase = std::move(passphrase);
