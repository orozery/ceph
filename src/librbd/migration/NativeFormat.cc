...
void NativeFormat<I>::open(Context* on_finish) {
  ...
  if (client_name_val.type() == json_spirit::str_type) {
    if (!cluster_name.has_value()) {
      lderr(cct) << "client_name cannot be used without cluster_name" << dendl;
      on_finish->complete(-EINVAL);
      return;
    }
    ...
    if (cluster_name.has_value()) {
      lderr(cct) << "client_name must be specified alongside cluster_name"
                 << dendl;
      on_finish->complete(-EINVAL);
      return;
    }
  ...
  if (cluster_name.has_value()) {
    ...
    int r = util::init_rados(cct, cluster_name.value(), client_name.value(),
                             "", "", "external cluster", args, &m_remote_rados,
                             true);
    if (r < 0) {
      lderr(cct) << "cannot initialize external cluster client:"
  ...
  librados::IoCtx io_ctx;
  int r = util::create_ioctx2(*rados_ptr, cct, "source image",
