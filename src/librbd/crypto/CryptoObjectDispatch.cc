...
struct C_UnalignedObjectWriteRequest : public Context {
    ...
    bool check_create_exclusive() {
      bool exclusive = ((write_flags & io::WRITE_FLAG_CREATE_EXCLUSIVE) != 0);
    ...
    void handle_read(int r) {
      ...
      if (!object_exists) {
        new_write_flags |=  io::WRITE_FLAG_CREATE_EXCLUSIVE;
    ...
    void handle_write(int r) {
      ldout(image_ctx->cct, 20) << "r=" << r << dendl;
      bool exclusive = write_flags & io::WRITE_FLAG_CREATE_EXCLUSIVE;
      bool restart = false;
