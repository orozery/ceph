...
void FlattenRequest<I>::flatten_crypto_header() {
  ...
  if (is_encryption_loaded) {
    data_offset =
            m_image_ctx->encryption_format->get_crypto()->get_data_offset();
  }
  m_image_ctx->image_lock.unlock_shared();
  ...
  uint64_t header_objects = Striper::get_num_objects(
          m_raw_image_ctx->layout, data_offset);
...
void FlattenRequest<I>::handle_metadata_remove(int r) {
  ...
  if (r < 0) {
    lderr(m_image_ctx->cct) << "error removing encryption metadata: "
                            << cpp_strerror(r) << dendl;
    m_return = r;
    close_raw_ctx();
    return;
...
void FlattenRequest<I>::crypto_flatten() {
  ...
  m_image_ctx->encryption_format->flatten(m_raw_image_ctx, ctx);
}
