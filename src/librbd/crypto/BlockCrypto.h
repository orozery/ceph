...
public:
    static BlockCrypto* create(CephContext* cct, DataCryptor<T>* data_cryptor,
                               IVGenerator* iv_generator, uint32_t block_size,
                               uint64_t data_offset) {
      return new BlockCrypto(cct, data_cryptor, iv_generator, block_size,
                             data_offset);
    }
    ...
    int encrypt(ceph::bufferlist* data, uint64_t image_offset,
                std::optional<io::ObjectMetadata>* metadata) override;
    int decrypt(ceph::bufferlist* data, uint64_t image_offset,
                std::optional<io::ObjectMetadata>* metadata) override;
...
private:
    ...
    int crypt(ceph::bufferlist* data, uint64_t image_offset,
              std::optional<io::ObjectMetadata>* metadata, CipherMode mode);
};
